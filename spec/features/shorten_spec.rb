...
feature "the shortening process" do
  ...
  scenario "do not allow shortening for a unregisterd user" do
    ...
    expect(page.find("input#shortened_url_original")[:placeholder]).to eq("Log in first!")
  end
  ...
  scenario "allow shortening for a unregisterd user" do
    ...
    expect(page.find("input#shortened_url_original")[:placeholder]).to eq("Your URL")
  end
  ...
  scenario "shorten URLs properly" do
    ...
    expect(page.find("input#to-url")[:value]).to eq("http://localhost:3000/redirect/9cuif51a")
  end
end